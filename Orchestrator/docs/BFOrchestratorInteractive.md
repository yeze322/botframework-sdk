...
the 'h' command for a brife help page on all the commandlets.
Below is what happened after issuing the 'h' commandlet.
```
Please enter a commandlet, "h" for help > h
  Commandlets: h, q, d, s, u, cu, i, ci, ni, cni, q, p, v,
    ...
    v   - validate the model and save analyses (validation report) to
          "experiment_evaluating_PrebuildDomain\orchestrator_predicting_set_summary.html"
    ...
    n   - create a new snapshot of model examples and save it to
          "experiment_evaluating_PrebuildDomain\orchestrator_predicting_snapshot_set.blu"
> "Current" utterance:          "hi"
> "Current" intent label array: "greeting"
> "New"     intent label array: ""
Please enter a commandlet, "h" for help >
...
For more detailed configutation information for Orchestrator, a user can issue the 'd' commandlet.
...
ground-truth intentl label, then that close utterance will be listed as an "ambiguous" prediction
...
so that the 'v' commandlet can create an evluation report with all the intent and utterances obfuscated.
